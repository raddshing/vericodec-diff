...
| T-LoRA | `https://github.com/ControlGenAI/T-LoRA.git` | `baselines/external/controlgenai_t-lora` | `available` | `yes` | Official SDXL path uses `stabilityai/stable-diffusion-xl-base-1.0`, `--resolution=1024`, and the shipped `tlora_env.yml`. This is the preferred RD-LoRA critical-path personalization baseline. |
| IntLoRA | `https://github.com/csguoh/IntLoRA.git` | `baselines/external/csguoh_intlora` | `available` | `no` | Official repo and environment are verified, but the substrate is quantized DreamBooth on Stable Diffusion 1.5, so it remains appendix-only. |
