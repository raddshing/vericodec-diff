...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Wrap the official diffusers SDXL LoRA script without reimplementing training."
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--run-name",
        default=None,
        help="Single-token run name under outputs/rd_lora/vanilla/<task_id>/.",
    )
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Opt in to actually launching the official diffusers training command.",
    )
    parser.add_argument(
        "--set",
        dest="set_values",
